 ...
 * keep passive update checks on the checkout's origin without triggering an
 * SSH prompt when a known public SSH remote can be probed over HTTPS.
 *
 * Run with: node --test electron/update-remote.test.ts
 * (Wired into npm test:desktop:platforms in package.json.)
 *
 * A background check still follows the user's selected origin. For a known
 * public GitHub/Gitee SSH URL, httpsProbeRemote supplies the equivalent HTTPS
 * URL so the check does not unexpectedly ask for an SSH hardware-key touch.
 */
...
test('httpsProbeRemote converts known GitHub and Gitee SSH remotes', () => {
  assert.equal(
    httpsProbeRemote('git@github.com:mikexia930/hermes-agent.git'),
    'https://github.com/mikexia930/hermes-agent.git'
  )
  assert.equal(
    httpsProbeRemote('ssh://git@gitee.com/supervadam/hermes-agent.git'),
    'https://gitee.com/supervadam/hermes-agent.git'
  )
})

test('httpsProbeRemote leaves HTTPS and unknown SSH remotes unchanged', () => {
  const https = 'https://github.com/mikexia930/hermes-agent.git'
  const unknown = 'git@gitlab.com:example/hermes-agent.git'
  assert.equal(httpsProbeRemote(https), https)
  assert.equal(httpsProbeRemote(unknown), unknown)
})
